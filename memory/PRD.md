...
- [x] **FINAL BACKLOG PR** — (1) local notifications: additive TimelineEntry.date + date input in add-moment sheet (native picker / web text), discreet-by-default copy + 'Show details' toggle, 9AM reminders + Sun-6PM weekly check-in, permission flows, resync everywhere, unit tests 14/14; (2) Face ID AppLockGate (launch + background→active, passcode fallback); (3) facts add/remove + kind picker in Edit; (4) per-person typed-delete (clears local refs, consistent list). Device verification (notif delivery + biometric prompt) tracked in RELEASE_CHECKLIST. ✅ iteration_19 + fix re-verification. User review pending.
- 🧊 **FEATURE FREEZE** (user-declared): no new features before release. "This week ✦" strip + structured birthday field parked in RELEASE_CHECKLIST Backlog/Ideas.
- ⏸️ HOLD: user is doing an independent code review of the full V2+PR4 diff. DO NOT start the release-prep pass (URL flip to api.lovli.in, remove EMERGENT_LLM_KEY/Anthropic default, remove ALLOW_TEST_LOGIN) until user confirms review is clean. After that: dev build for device verification (notifications, Face ID).
