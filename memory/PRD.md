...
- [x] **PR-V2-3.1** — Intent screen additions: REPLY LANGUAGE per-generation override chips + WHO'S THIS ABOUT? person chips (mini gradient avatars, "No one" default, tap-toggle, hidden when no memory cards). UI-only → existing `language` / `memory_card_id` params. CTA pinned below scrollable chip area. ✅ self-tested (screenshots), user review pending
